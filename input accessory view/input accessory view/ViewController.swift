...
class ViewController: UIViewController , UIImagePickerControllerDelegate,UINavigationControllerDelegate ,AVAudioRecorderDelegate,AVAudioPlayerDelegate {
    ...
    //MARK: -play progress
    var sendRecordedAuido = AnimationView()
    lazy var playProgress : UIView = {
        ...
        sendRecordedAuido = .init(name: "send")
        sendRecordedAuido.contentMode = .scaleAspectFill
        let send = UITapGestureRecognizer(target: self, action:  #selector (sendRecordedSound(_:)))
        sendRecordedAuido.addGestureRecognizer(send)
        
        
        v.addSubview(sendRecordedAuido)
        
        sendRecordedAuido.anchor(top: nil, left: nil, bottom: nil, rigth: v.rightAnchor, marginTop: 0, marginLeft: 0, marginBottom: 0, marginRigth: 8, width: 40, heigth: 40)
        sendRecordedAuido.centerYAnchor.constraint(equalTo: v.centerYAnchor).isActive = true
